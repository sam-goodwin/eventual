    ...
    const workflowHook: ExecutionWorkflowHook = {
      registerEventualCall<E extends EventualPromise<any>>(call: EventualCall) {
        try {
           ...
           * Note: this contract is too abstracted, call => command => schedule event
           *       where the scheduled event is generated by the orchestrator.
           *       I'd love to be able to match calls to call or something
           *       instead of matching events to calls in {@link }.
