...
import { RegisterSignalHandlerCallFactory } from "./call-executors-and-factories/register-signal-handler-call.js";
...
export interface EventualFactory<C extends Call = Call> {
  createEventualDefinition(call: C): EventualDefinition<CallOutput<C>>;
}
...
export class AllWorkflowEventualFactory implements EventualFactory {
  ...
  public createEventualDefinition(call: Call): EventualDefinition<any> {
    ...
    if (executor) {
      return executor.createEventualDefinition(call);
    }
...
export function createDefaultEventualFactory(): AllWorkflowEventualFactory {
  ...
  return new AllWorkflowEventualFactory({
    ...
    RegisterSignalHandlerCall: new RegisterSignalHandlerCallFactory(),
